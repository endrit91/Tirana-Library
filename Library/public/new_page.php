...
<?php
  // Can't add a new page unless we have a menu as a parent!
if (!$current_menu) {
...
if (isset($_POST['submit']) && isset ($_POST['visible'])) {
  ...
  if (empty($errors)) {
    ...
    $query .= "  menu_id, page_name, position, visible, content, img_source ";
    $query .= ") VALUES (";
    $query .= "  {$menu_id}, '{$page_name}', {$position}, {$visible}, '{$content}', '{$img_source}' ";
    $query .= ")";
...
<div id="main">
  ...
  <div id="page">
    ...
    <form action="new_page.php?menu=<?php echo urlencode($current_menu["id"]); ?>" method="post">
      ...
      <p>IMG Source:
        <input type="text" name="img_source" value="" />
      </p>
      <p>Content:<br />
